...
const useStyles = makeStyles({
 ...
 function StickyHeadTable(props) {
  ...
  const onDeleteClick=(index)=>{
    console.log('berhasil Delete' + index)
  ...
  const renderTable=()=>{
    return product.map((val,index)=>{
      console.log(val.tanggalmulai + 'ini tnaggal mulai')
      console.log(today + ' ini today')
      if(today >= val.tanggalmulai){
        console.log(index + ' expired')
        return(
          <TableRow key={val.id}>
              ...
              <TableCell><ReadMoreReact text={val.deskripsi} min={10} ideal={20} ReadMoreReact="Read More" /></TableCell>
              <TableCell>EXPIRED</TableCell>
              <TableCell>
                <span style={{fontSize:30}} className='text-danger mr-3' onClick={()=>onDeleteClick(val.id)}><MdDeleteForever/></span>
